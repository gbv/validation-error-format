...
- the locator MAY have either a field `errors` with an array of [**errors**](#errors) within the located element or a field `reports` with an array of [**reports**](#reports) for the located element. Both contain **nested errors**.
...
- the report MAY have field `compliances` with an array of [positions], optionally followed by the value `null` to indicate an incomplete list. Compliances list positions
...
```{#lst-10 .json lst-cap="An error with an nested error report"}
{
