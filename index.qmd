...
- mandatory field `format` with the name of a **[locator format](#sec-locator-formats)**, being a non-empty string
...
Errors in the `errors` field are called **nested errors**. Nested errors allow to reference locations within nested documents (@lst-nested-example and @lst-nested-example-2):
