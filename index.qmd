...
Last but not least the UTF-8 encoding could be invalid. The document given in @lst-2 contains an invalid byte code when decoded as UTF-8. Some validators may replace the byte with the Unicode replacement character `U+FFFD` but the resulting Unicode string is invalid JSON syntax still.

::: {#lst-2 lst-cap="Invalid JSON on multiple levels"}

+---------------:+:------:+:------:+:------:+:------:+:------:+:------:+:------:+:------:+:------:+:------:+
| **Byte**       |`7b`    |`22`    |`c3`    |`a5`    |`22`    |`3a`    |`22`    |`3a`    |`c0`    |`7d`    |
+----------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+
...
`iiif`          | ...                   | ...
`rfc7111`       | ...                   | see [RFC 7111](https://tools.ietf.org/html/rfc7111)
...             | ...                   | ...
